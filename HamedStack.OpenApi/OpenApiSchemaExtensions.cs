...
using HamedStack.OpenApi.Helpers;
using HamedStack.OpenApi.Models;
using HamedStack.OpenApi.Settings;
using Humanizer;
...
public static partial class OpenApiExtensions
{
    ...
    public static string ToCSharpClass(this OpenApiSchema openApiSchema, string rootClassName = "Root")
    {
        return GenerateCSharpClass(openApiSchema, rootClassName, new Dictionary<string, OpenApiSchema>());
    }
    private static string GenerateCSharpClass(this OpenApiSchema rootSchema, string rootClassName, IReadOnlyDictionary<string, OpenApiSchema> knownSchemas)
    {
        var generated = new HashSet<string>();
        var classBuilder = new StringBuilder();

        GenerateClass(rootSchema, rootClassName, classBuilder, knownSchemas, generated);

        return ExtractClasses(classBuilder.ToString());

        void GenerateClass(OpenApiSchema schema, string className, StringBuilder builder, IReadOnlyDictionary<string, OpenApiSchema> schemas, HashSet<string> generatedString)
        {
            if (generatedString.Contains(className))
                return;

            generatedString.Add(className);
            ...
            {
                var propertyType = GetTypeFromOpenApiSchema(property.Value, schemas, ToPascalCase(property.Key), generatedString);
                builder.AppendLine($"    public {propertyType} {ToPascalCase(property.Key)} {{ get; set; }}");
            }
        ...
        string GetTypeFromOpenApiSchema(OpenApiSchema schema, IReadOnlyDictionary<string, OpenApiSchema> schemas, string suggestedName, HashSet<string> generatedTypes)
        {
            ...
            switch (schema.Type)
            {
                ...
                case "array":
                    var itemType = GetTypeFromOpenApiSchema(schema.Items, schemas, suggestedName, generatedTypes);
                    if (!IsPrimitiveOrKnownType(itemType))
                        GenerateClass(schema.Items, itemType, classBuilder, schemas, generated);
                    return $"List<{itemType}>";

                case "object":
                    if (!generatedTypes.Contains(suggestedName))
                        GenerateClass(schema, suggestedName, classBuilder, schemas, generatedTypes);
                    return suggestedName;

                default:
                    throw new InvalidOperationException($"Unhandled OpenApiSchema type: {schema.Type}");
            }
        }
        string ToPascalCase(string input)
        {
            ...
            return input[..1].ToUpper(CultureInfo.InvariantCulture) + input[1..];
        }

        bool IsPrimitiveOrKnownType(string csharpTypeName)
        {
            ...
            return knownTypes.Contains(csharpTypeName) || knownTypes.Contains($"{csharpTypeName}?");
        }

        string ExtractClasses(string text)
        {
            var textLines = text.Split(new[] { Environment.NewLine }, StringSplitOptions.TrimEntries).Where(x => !string.IsNullOrWhiteSpace(x)).ToList();

            var classes = textLines.Where(x => x.StartsWith("public class")).Reverse().ToList();
            var nestedClasses = new List<string>();
            foreach (var cls in classes)
            {
                var extractedClass = textLines.Between(x => x == cls, y => y == "}", true, out var list);
                var sb = new StringBuilder();
                foreach (var extracted in extractedClass)
                {
                    var space = extracted == "{" || extracted == "}" || extracted.StartsWith("public class") ? "" : "\t";
                    sb.AppendLine(space + extracted);
                }
                nestedClasses.Add(sb.ToString());
                textLines = list.ToList();
            }

            var result = nestedClasses.Aggregate((x, y) => x + Environment.NewLine + y).Trim();
            return result;
        }
    }
    private static string GenerateJsonExample(this OpenApiSchema schema, IReadOnlyDictionary<string, OpenApiSchema> knownSchemas, Func<string, string, object>? defaultValueProvider = null)
    {
        ...
        void WriteJsonExample(Utf8JsonWriter writer, OpenApiSchema s)
        {
            ...
            {
                case "string":
                    var exampleString = "example_string";
                    switch (s.Format)
                    {
                        case "date":
        ...
        void WriteOpenApiValue(Utf8JsonWriter writer, IOpenApiAny openApiAnyValue)
        {
            switch (openApiAnyValue)
            {
                ...
                case OpenApiByte byteVal:
                    writer.WriteStringValue(byteVal.Value);
                    break;
                ...
                case OpenApiPassword passwordVal:
                    writer.WriteStringValue(passwordVal.Value);
                    break;
                case OpenApiBinary binaryVal:
                    writer.WriteStringValue(Encoding.UTF8.GetString(binaryVal.Value));
                    break;
                case OpenApiArray arrayVal:
                    writer.WriteStartArray();
                    foreach (var item in arrayVal)
                    {
                        WriteOpenApiValue(writer, item);
                    }
                    writer.WriteEndArray();
                    break;
                case OpenApiObject objVal:
                    ...
                    foreach (var item in objVal)
                    {
                        ...
                        WriteOpenApiValue(writer, item.Value);
                    }
                    writer.WriteEndObject();
                    break;
                default:
    ...
    public static string ToTypeScriptType(this OpenApiSchema openApiSchema)
    {
        return GenerateTypeScriptType(openApiSchema, new Dictionary<string, OpenApiSchema>());
    }

    private static string GenerateTypeScriptType(OpenApiSchema schema, IReadOnlyDictionary<string, OpenApiSchema> knownSchemas)
    {
        var sb = new StringBuilder();
        var generatedTypes = new HashSet<string>();

        GenerateTypeScriptDefinition(sb, schema);

        return sb.ToString();

        void GenerateTypeScriptDefinition(StringBuilder builder, OpenApiSchema s)
        {
            if (s.Reference != null && knownSchemas.ContainsKey(s.Reference.Id))
            {
                var typeName = s.Reference.Id;
                ...
                builder.Append(typeName);
                builder.AppendLine(" = ");
                GenerateTypeScriptDefinition(builder, knownSchemas[s.Reference.Id]);
                builder.AppendLine(";");
                return;
            }
            ...
            {
                builder.Append(string.Join(" | ", s.Enum.Select(e =>
                    e.ToString())));
                return;
            }

            void HandleCompositeType(StringBuilder b, IEnumerable<OpenApiSchema> schemas, string delimiter)
            {
                b.Append(string.Join(delimiter, schemas.Select(o =>
                {
                    var temp = new StringBuilder();
                    GenerateTypeScriptDefinition(temp, o);
            ...
            {
                HandleCompositeType(builder, s.OneOf, " | ");
                return;
            }

            if (s.AllOf is { Count: > 0 })
            {
                HandleCompositeType(builder, s.AllOf, " & ");
                return;
            }
            ...
            switch (s.Type)
            {
                ...
                case "array":
                    ...
                    GenerateTypeScriptDefinition(builder, s.Items);
                    builder.Append(">");
                    break;
                ...
                case "object":
                    ...
                    if (s.Properties != null)
                    {
                        foreach (var property in s.Properties)
                        {
                            ...
                            GenerateTypeScriptDefinition(builder, property.Value);
                            if (s.Required == null || !s.Required.Contains(property.Key))
                            {
                                builder.Append("?");
                            }
                            builder.AppendLine(";");
                    ...
                    if (s is { AdditionalPropertiesAllowed: true, AdditionalProperties: not null })
                    {
                        builder.Append("  [key: string]: ");
                        GenerateTypeScriptDefinition(builder, s.AdditionalProperties);
                        builder.AppendLine(";");
                    }
                    builder.Append("}");
                    break;
